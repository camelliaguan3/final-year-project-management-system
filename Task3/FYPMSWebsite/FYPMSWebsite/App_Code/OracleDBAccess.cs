...
namespace FYPMSWebsite.App_Code
{
    ...
    public class OracleDBAccess
    {
        ...
        private readonly OracleConnection myOracleDBConnection = new OracleConnection(ConfigurationManager.ConnectionStrings["FYPMSConnectionString"].ConnectionString);
        ...
        {
            isSqlError = true;
            sqlErrorMessage = $"*** SQL error in {source}: {error}.";
        }
        ...
        public decimal GetAggregateValue(string source, string sql)
        {
            ...
            try
            {
                ...
                if (aggregateValue == DBNull.Value)
                {
                    result = 0;
                }
        ...
        public bool SetData(string TODO, string sql)
        {
            ...
            if (UpdateData(TODO, sql, trans)) // The update succeeded.
            {
                CommitTransaction(TODO, trans); return true;
            }
            else // The update failed.
            {
                RollbackTransaction(TODO, trans); return false;
            }
        }
        ...
        public OracleTransaction BeginTransaction(string source)
        {
            ...
            try
            {
                ...
                else // The connection is already open.
                {
                    result = myOracleDBConnection.BeginTransaction();
                }
            }
        ...
        public void RollbackTransaction(string source, OracleTransaction trans)
        {
            try
            {
                if (myOracleDBConnection.State == ConnectionState.Open)
